...
export default createSelector(
  ...
  (surfaces, grids, networks, cutoff) =>
    surfaces.map((surface, index) =>
      grids.map(grid => surface && surface.data && grid.data && networks[index] && networks[index].query
        ? accessibilityForGrid({surface: surface.data, grid, query: networks[index].query, cutoff})
        : -1
      )
    )
)
